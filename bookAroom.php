...
// Check if the 'booknow' button was clicked
...
if (isset($_POST['arrival'], $_POST['departure'], $_POST['accomodation'])) {
    ...
    <div class="col">
        ...
        <div class="card-columns">
            ...
            $query = "SELECT * FROM `tblroom` r ,`tblaccomodation` a 
                      WHERE r.`ACCOMID`=a.`ACCOMID` 
                      AND `ACCOMODATION`='" . $_POST['accomodation'] . "' 
                      AND `NUMPERSON` = " . $_POST['person'];
            $mydb->setQuery($query);
            ...
            foreach ($cur as $result) { 
                ...
                $resNum = isset($result->OROOMNUM) ? $result->OROOMNUM - count($curs) : 0;

                $stats = $mydb->executeQuery();
                $rows = mysqli_fetch_assoc($stats);
                $status = $rows ? $rows['STATUS'] : null;
                ...
                if ($resNum == 0) {
                    if ($status == 'Confirmed') {
                        ...
                        $img_title = '<figcaption class="img-title-active"><h5>Book!</h5></figcaption>';
                    } else {
            ...
            <form method="POST" action="index.php?p=accomodation">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="rooms_title"><h2><?php echo $result->ROOM . ' ' . $result->ACCOMODATION; ?></h2></div>
                        ...
                        <div class="rooms_price"><?php echo $result->PRICE; ?>/Night</div>
                        <?php echo $btn; ?> 
                    </div>
                </div>
            </form>
